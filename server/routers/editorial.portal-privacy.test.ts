...
describe("proteção pública de Coberturas contratadas", () => {
  ...
  it("retira conteúdo em quarentena do portal sem exigir destruição do arquivo", () => {
    expect(canExposeOnPublicPortal({ ...publishedEditorial, quarantinedAt: new Date() })).toBe(false);
  });
