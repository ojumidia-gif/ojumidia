...
| `GET /api/oauth/callback`                   |             Sim |           Não | Valida nonce OAuth                     | Não                                          | Cria sessão somente após troca válida de código.                    |
| `GET /api/editorial/events`                 |             Sim |           Não | Nenhum                                 | Eventos públicos editoriais                  | SSE para atualização do portal.                                     |
| `POST /api/media/upload`                    |             Não |           Sim | Criador, editor, aprovador ou admin    | Validado no backend para parceiro/território | Limite de 16 MB, checksum, sessão rastreável e publicação separada. |
| `/manus-storage/*`                          |      Controlado |      Indireto | URL assinada                           | Chave de storage                             | Redireciona para URL temporária; não usa disco Render.              |
