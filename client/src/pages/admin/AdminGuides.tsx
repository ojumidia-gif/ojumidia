      ...
      <div className="grid gap-4 md:grid-cols-2">
        {siteDestinations.map(dest => (
          <article key={dest.id} className="admin-card p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-oju-terra-suave">{dest.how}</p>
            <ol className="mt-4 grid gap-2 text-sm">
              {dest.steps.map((step, index) => (
                <li key={step} className="rounded-xl bg-oju-papel px-3 py-2"><strong>{index + 1}.</strong> {step}</li>
              ))}
            </ol>
            <Link href={dest.adminHref} className="mt-4 inline-flex rounded-full bg-oju-verde-profundo px-4 py-2 text-sm font-semibold text-white">Começar {dest.label}</Link>
          </article>
        ))}
      </div>
    </AdminPage>
