...
export default function TaxonomiesAdmin() {
  ...
  return <AdminPage eyebrow={territoriesFocus ? "Portal → Territórios" : "Territórios e taxonomias"} title={territoriesFocus ? "Cadastrar, editar e publicar no mapa." : "Criar, relacionar e documentar."} action={null}>
    ...
    {selectedId && <section className="admin-card mt-7 p-5"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-[#806817]">Conteúdos relacionados</p><p className="mt-1 text-sm text-[#655e52]">Coberturas, fotografias ou histórias que receberam este vínculo documental.</p></div><Button variant="ghost" size="sm" onClick={() => setSelectedId(null)}><X className="h-4 w-4" /></Button></div><div className="mt-4 grid gap-3 md:grid-cols-2">{linked?.length ? linked.map(item => <div key={item.id} className="rounded-xl border border-[#242017]/10 bg-white p-4"><p className="font-medium">{item.title}</p><p className="mt-1 text-xs text-[#655e52]">{item.contentKind} · {item.status}</p><div className="mt-3 flex gap-2"><Button asChild size="sm" variant="outline"><Link href={`/admin/editar/${item.id}`}>Editar conteúdo</Link></Button>{item.status === "Publicada" && <Button asChild size="sm"><Link href={`/admin/preview/${item.id}`}>Revisar</Link></Button>}</div></div>) : <p className="text-sm text-[#655e52]">Ainda não há conteúdos relacionados. Abra uma Cobertura e selecione este item em Relações documentais.</p>}</div></section>}
    {mediaTarget && <div><div className="mt-7 flex justify-end"><Button size="sm" variant="outline" onClick={() => setMediaTargetId(null)}><X className="mr-1 h-3.5 w-3.5" />Fechar mídias</Button></div><TaxonomyMediaPanel taxonomy={mediaTarget} /></div>}
    <div className="mt-7 grid gap-5 lg:grid-cols-2">{isLoading ? <p>Carregando itens...</p> : grouped.map(([label, entries]) => <section key={label} className="admin-card p-5"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[.15em] text-[#806817]">{label}</p><Button size="sm" variant="outline" onClick={() => startNew(label)}><Plus className="mr-1 h-3.5 w-3.5" />Adicionar</Button></div><div className="mt-4 space-y-2">{entries.length ? entries.map(item => <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-[#242017]/10 bg-white px-3 py-3"><div><p className="text-sm font-medium">{item.name}</p>{item.description && <p className="mt-1 text-xs text-[#655e52]">{item.description}</p>}</div><div className="flex shrink-0 gap-1"><Button size="icon" variant="ghost" title="Ver conteúdos relacionados" onClick={() => setSelectedId(item.id)}><FolderOpen className="h-4 w-4" /></Button><Button size="icon" variant="ghost" title="Adicionar imagem ou vídeo" onClick={() => { setMediaTargetId(item.id); setSelectedId(null); window.scrollTo({ top: 0, behavior: "smooth" }); }}><ImagePlus className="h-4 w-4" /></Button><Button size="icon" variant="ghost" title="Editar item" onClick={() => edit(item)}><Edit3 className="h-4 w-4" /></Button><Button size="icon" variant="ghost" title="Excluir item" onClick={() => { if (window.confirm(`Remover “${item.name}”? Os conteúdos não serão excluídos.`)) remove.mutate({ id: item.id }); }}><Trash2 className="h-4 w-4 text-[#8b4d24]" /></Button></div></div>) : <div className="rounded-xl border border-dashed border-[#242017]/15 bg-white/60 p-4"><p className="text-sm text-[#756e60]">Nenhum item cadastrado.</p><Button className="mt-3" size="sm" onClick={() => startNew(label)}><Plus className="mr-1 h-3.5 w-3.5" />Cadastrar {label}</Button></div>}</div></section>)}</div>
