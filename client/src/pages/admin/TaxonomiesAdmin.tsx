...
export default function TaxonomiesAdmin() {
  ...
  return <AdminPage eyebrow={territoriesFocus ? "Portal → Territórios" : "Territórios e taxonomias"} title={territoriesFocus ? "Cadastrar, editar e publicar no mapa." : "Criar, relacionar e documentar."} action={null}>
    {territoriesFocus ? <p className="-mt-4 mb-4 max-w-3xl text-sm leading-6 text-oju-terra-suave">Cadastre o território. Depois ligue histórias e coberturas no conteúdo. Visibilidade no mapa é opcional e só com autorização da casa. Você edita só o que cadastrou; o catálogo nacional fica com a Equipe Ojú.</p> : null}
    ...
    <section className="admin-card p-6">
      ...
      <form className="mt-6 grid gap-4 md:grid-cols-2" onSubmit={event => { event.preventDefault(); const mapData = dimension === "Território" ? { latitude: latitude || undefined, longitude: longitude || undefined, mapVisibility } : {}; if (editingId) update.mutate({ id: editingId, name, description: description || null, parentId: parentId ? Number(parentId) : null, ...mapData }); else create.mutate({ dimension, name, description: description || undefined, parentId: parentId ? Number(parentId) : undefined, ...mapData }); }}>
        ...
        <label className="grid gap-2 text-sm font-medium">Relacionado a <span className="font-normal text-oju-terra-suave">(opcional)</span><select value={parentId} onChange={event => setParentId(event.target.value)} className="h-10 rounded-md border bg-white px-3"><option value="">Sem relação superior</option>{parents.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
        <label className="grid gap-2 text-sm font-medium">Descrição ou contexto <span className="font-normal text-oju-terra-suave">(opcional)</span><Textarea value={description} onChange={event => setDescription(event.target.value)} placeholder="Explique como este item deve aparecer e ser encontrado no acervo." /></label>
        ...
        <div className="flex justify-end md:col-span-2"><Button disabled={create.isPending || update.isPending} className="bg-oju-verde text-oju-branco">{editingId ? "Salvar alterações" : `Cadastrar ${dimension}`}</Button></div>
      </form>
    </section>
    {selectedId && <section className="admin-card mt-7 p-5"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-[#806817]">Conteúdos relacionados</p><p className="mt-1 text-sm text-oju-terra-suave">Só aparecem os conteúdos da sua carteira. Os de outros admins não entram aqui.</p></div><Button variant="ghost" size="sm" onClick={() => setSelectedId(null)}><X className="h-4 w-4" /></Button></div><div className="mt-4 grid gap-3 md:grid-cols-2">{linked?.length ? linked.map(item => <div key={item.id} className="rounded-xl border border-oju-terra/10 bg-white p-4"><p className="font-medium">{item.title}</p><p className="mt-1 text-xs text-oju-terra-suave">{item.contentKind} · {item.status}</p><div className="mt-3 flex gap-2"><Button asChild size="sm" variant="outline"><Link href={`/admin/editar/${item.id}`}>Editar conteúdo</Link></Button>{item.status === "Publicada" && <Button asChild size="sm"><Link href={`/admin/preview/${item.id}`}>Revisar</Link></Button>}</div></div>) : <p className="text-sm text-oju-terra-suave">Nenhum conteúdo seu está ligado a este item ainda.</p>}</div></section>}
    ...
    <div className="mt-7 grid gap-5 lg:grid-cols-2">{isLoading ? <p>Carregando itens...</p> : grouped.map(([label, entries]) => <section key={label} className="admin-card p-5"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[.15em] text-[#806817]">{label}</p><Button size="sm" variant="outline" onClick={() => startNew(label)}><Plus className="mr-1 h-3.5 w-3.5" />Adicionar</Button></div><div className="mt-4 space-y-2">{entries.length ? entries.map(item => <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-oju-terra/10 bg-white px-3 py-3"><div><p className="text-sm font-medium">{item.name}</p>{item.description && <p className="mt-1 text-xs text-oju-terra-suave">{item.description}</p>}{!canWrite(item) && <p className="mt-1 text-[11px] text-[#806817]">Catálogo compartilhado — ligue nas suas publicações; não edita.</p>}</div><div className="flex shrink-0 gap-1"><Button size="icon" variant="ghost" title="Ver conteúdos relacionados" onClick={() => setSelectedId(item.id)}><FolderOpen className="h-4 w-4" /></Button>{canWrite(item) ? <><Button size="icon" variant="ghost" title="Adicionar imagem ou vídeo" onClick={() => { setMediaTargetId(item.id); setSelectedId(null); window.scrollTo({ top: 0, behavior: "smooth" }); }}><ImagePlus className="h-4 w-4" /></Button><Button size="icon" variant="ghost" title="Editar item" onClick={() => edit(item)}><Edit3 className="h-4 w-4" /></Button><Button size="icon" variant="ghost" title="Excluir item" onClick={() => { if (window.confirm(`Remover “${item.name}”? Os conteúdos não serão excluídos.`)) remove.mutate({ id: item.id }); }}><Trash2 className="h-4 w-4 text-[#8b4d24]" /></Button></> : null}</div></div>) : <div className="rounded-xl border border-dashed border-oju-terra/15 bg-white/60 p-4"><p className="text-sm text-[#756e60]">Nenhum item cadastrado.</p><Button className="mt-3" size="sm" onClick={() => startNew(label)}><Plus className="mr-1 h-3.5 w-3.5" />Cadastrar {label}</Button></div>}</div></section>)}</div>
