...
function videoDuration(file: File) {
  return new Promise<number>((resolve, reject) => {
    ...
    video.onerror = () => {
      URL.revokeObjectURL(url);
      reject(
        new Error(`Não foi possível ler a duração de ${file.name}.`)
      );
    };
...
export function CoverageMediaPanel({
  publicationId,
  coverageTitle,
  contentKind,
  documentaryPhotos = false,
  existingMedia,
  eventNames = [],
}: {
  ...
  const detach = trpc.editorial.detachMedia.useMutation({
    onSuccess: () => {
      toast.success(
        "Mídia removida do conteúdo. Uma vaga foi liberada."
      );
      refresh();
    },
    onError: (error) => toast.error(error.message),
  ...
  const approve = trpc.media.approveUpload.useMutation({
    onError: (error) => toast.error(error.message),
  });

  const photoCount = existingMedia.filter(
    (media) => media.mediaType === "foto"
  ).length;
  ...
  const hasRoom = (
    ...
    mediaType === "foto"
      ? photoCount + additions <= MAX_PHOTOS
      : !documentaryPhotos &&
        videoCount + additions <= MAX_VIDEOS;
  ...
  async function upload(event: React.FormEvent) {
    ...
    if (
      !hasRoom("foto", photoFiles) ||
      !hasRoom("vídeo", videoFiles)
    ) {
      return toast.error(
        `Limite absoluto: ${MAX_PHOTOS} fotos e ${MAX_VIDEOS} vídeos por conteúdo.`
      );
    }
    ...
    try {
      for (
        ...
        if (
          ...
          throw new Error(
            "O servidor não retornou os dados necessários do upload."
          );
        }

        const verifiedDuration =
          uploaded.durationSeconds ?? duration;
        ...
        const media = await create.mutateAsync({
          ...
          projectCoverage: coverageTitle,
          publicationAllowed: true,
          durationSeconds: verifiedDuration,
          uploadId: uploaded.uploadId,
        });

        // 3. Aprova a mídia para uso editorial
        await approve.mutateAsync({
          id: media.id,
        });

        // 4. Vincula a mídia à publicação
  ...
  const contextualInput = documentaryPhotos ? (
    <>
      ...
      <label className="grid gap-2 text-sm font-medium">
        ...
        <Input
          ...
          placeholder="Ex.: Terreiro, bairro, cidade"
        />
      </label>

      <label className="grid gap-2 text-sm font-medium">
        Biografia viva / contexto
        <Textarea
          required
          value={biography}
          onChange={(event) =>
          ...
          placeholder="Quem aparece, o que está acontecendo e por que esta imagem importa."
        />
      </label>
    </>
  ) : null;
            ...
            >
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  variant="outline"
                  disabled={
                    !contextualFieldsReady ||
                    !hasRoom(media.mediaType)
                  }
                  onClick={() =>
                    attach.mutate({
                      ...attachmentData,
