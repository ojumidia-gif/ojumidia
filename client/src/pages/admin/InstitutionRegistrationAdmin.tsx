...
export default function InstitutionRegistrationAdmin() {
  ...
  const create = trpc.community.createInstitution.useMutation({
    onSuccess: () => { setSaved(true); toast.success("Perfil comunitário salvo com consentimento registrado."); },
  ...
  return <AdminPage eyebrow="Perfis comunitários autorizados" title="Cadastrar com consentimento visível.">
    <div className="grid max-w-5xl gap-7 lg:grid-cols-[1fr_.85fr]">
      <form className="admin-card grid gap-4 p-6" onSubmit={event => {
        ...
        <div className="flex items-start gap-3"><div className="rounded-xl bg-[#f6d978] p-3"><Landmark className="h-5 w-5" /></div><div><h2 className="font-serif text-3xl">Perfil comunitário documental</h2><p className="mt-1 text-sm leading-6 text-[#655e52]">O registro permanece em rascunho até a revisão administrativa. A contribuição por visibilidade é opcional, posterior e nunca compra curadoria.</p></div></div>
        {saved ? <p className="rounded-lg bg-[#d8eadc] p-3 text-sm text-[#2c683b]">Perfil salvo com consentimento registrado. A camada de visibilidade pode ser gerida depois em “Visibilidade institucional”.</p> : null}
        ...
        <Button disabled={!consent || create.isPending} className="bg-[#242017] text-white">{create.isPending ? "Salvando..." : "Salvar perfil com consentimento"}</Button>
      </form>
