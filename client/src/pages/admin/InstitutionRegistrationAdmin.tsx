  ...
  return <AdminPage eyebrow="Perfis comunitários autorizados" title="Cadastrar com consentimento visível.">
    <div className="grid max-w-5xl gap-7 lg:grid-cols-[1fr_.85fr]">
      <form className="admin-card grid gap-4 p-6" onSubmit={async event => {
        ...
        <div className="flex items-start gap-3"><div className="rounded-xl bg-[#f6d978] p-3"><Landmark className="h-5 w-5" /></div><div><h2 className="font-serif text-3xl">Perfil comunitário documental</h2><p className="mt-1 text-sm leading-6 text-oju-terra-suave">Consentimento obrigatório. Depois: Publicar no site. Visibilidade paga nunca compra curadoria.</p></div></div>
        ...
        <p className="-mt-2 text-xs leading-5 text-oju-terra-suave">Os dois campos de referência são livres: a própria pessoa ou instituição define a forma como deseja ser identificada.</p>
        ...
        <div className="flex flex-wrap gap-3">
          <Button disabled={!consent || create.isPending} className="bg-oju-verde text-oju-branco">{create.isPending ? "Salvando..." : "Salvar com consentimento"}</Button>
          {savedId ? <Button type="button" disabled={publish.isPending} className="bg-oju-verde text-oju-branco" onClick={() => publish.mutate({ id: savedId, status: "Publicada" })}>Publicar no site</Button> : null}
        </div>
      </form>
      <aside className="h-fit rounded-2xl border border-oju-terra/10 bg-[#f7f3e9] p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Proteção aplicada</p><h2 className="mt-3 font-serif text-3xl">A comunidade define sua presença.</h2><ul className="mt-5 grid gap-3 text-sm leading-6 text-oju-terra-suave"><li>O endereço começa como <b>Não divulgar</b>.</li><li>O mapa só recebe ponto com localização pública e coordenadas autorizadas.</li><li>O contato só é exibido com marcação específica.</li><li>O consentimento é registrado junto ao perfil.</li><li>Visibilidade institucional não altera busca, curadoria nem publicação editorial.</li></ul></aside>
    </div>
  </AdminPage>;
}
