      ...
      {published.map(item => {
        ...
        return (
          <article key={item.id} className="admin-card grid gap-4 p-5 lg:grid-cols-[96px_1fr_170px_90px_90px_auto]">
            <div className="h-24 overflow-hidden rounded-xl bg-[#eee9dc]">
              {item.coverUrl ? <img src={item.coverUrl} alt="" className="h-full w-full object-cover" /> : <p className="flex h-full items-center justify-center px-2 text-center text-[11px] text-[#655e52]">Sem capa</p>}
            </div>
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <span className="rounded-full bg-[#eee9dc] px-2 py-1 text-[11px]">{item.contentKind}</span>
                {onHome ? <span className="rounded-full bg-[#d8eadc] px-2 py-1 text-[11px] font-semibold text-[#2c683b]">Na Home</span> : <span className="rounded-full bg-[#eee9dc] px-2 py-1 text-[11px]">Fora da Home</span>}
              </div>
              <label className="mt-3 flex items-start gap-2 text-sm">
                ...
                <span><strong>Mostrar em Histórias recentes</strong><span className="mt-1 block text-xs text-[#655e52]">Usa a foto de capa deste conteúdo.</span></span>
              </label>
            </div>
            <label className="grid gap-1 text-xs font-semibold uppercase tracking-wider text-[#655e52]">Posição<select value={state.placement} onChange={event => setChanges({ ...changes, [item.id]: { ...state, placement: event.target.value as typeof options[number], featured: event.target.value !== "Nenhum" } })} className="h-9 rounded-md border bg-white px-2 text-sm normal-case tracking-normal text-[#242017]">{options.map(option => <option key={option}>{option}</option>)}</select></label>
            <label className="grid gap-1 text-xs font-semibold uppercase tracking-wider text-[#655e52]">Ordem<input type="number" min="0" max="99" value={state.homeOrder} onChange={event => setChanges({ ...changes, [item.id]: { ...state, homeOrder: Number(event.target.value) } })} className="h-9 rounded-md border bg-white px-2 text-sm" /></label>
            <label className="grid gap-1 text-xs font-semibold uppercase tracking-wider text-[#655e52]">Relevância<input type="number" min="0" max="100" value={state.relevance} onChange={event => setChanges({ ...changes, [item.id]: { ...state, relevance: Number(event.target.value) } })} className="h-9 rounded-md border bg-white px-2 text-sm" /></label>
            <div className="flex flex-col justify-end gap-2">
              <label className="grid gap-1 text-[11px] font-semibold uppercase tracking-wider text-[#655e52]">Expira<input type="datetime-local" value={state.highlightExpiresAt} onChange={event => setChanges({ ...changes, [item.id]: { ...state, highlightExpiresAt: event.target.value } })} className="h-9 rounded-md border bg-white px-2 text-sm" /></label>
              <div className="flex gap-2">
                <Button asChild variant="outline"><Link href={`/admin/home-preview/${item.id}`}>Prévia</Link></Button>
                <Button disabled={save.isPending} onClick={() => save.mutate({ id: item.id, manualFeatured: state.featured && state.placement !== "Nenhum", relevance: state.relevance, homePlacement: state.featured ? (state.placement === "Nenhum" ? "Recomendado" : state.placement) : "Nenhum", homeOrder: state.homeOrder, highlightExpiresAt: state.highlightExpiresAt ? new Date(state.highlightExpiresAt) : null })} className="bg-[#242017] text-white">Salvar</Button>
              </div>
            </div>
          </article>
        );
