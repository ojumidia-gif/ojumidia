...
export default function PartnersAdmin() {
  ...
  return <AdminPage eyebrow="Governança territorial" title="Parceiros Ojú e escopos operacionais." action={<div className="flex flex-wrap gap-2"><Link href="/admin/publicacoes" className="rounded-full border border-[#242017] px-4 py-2 text-sm font-bold">Conteúdos</Link><Link href="/admin/midias" className="rounded-full border border-[#242017] px-4 py-2 text-sm font-bold">Mídias</Link><Link href="/admin/solicitacoes" className="rounded-full border border-[#242017] px-4 py-2 text-sm font-bold">Comercial</Link><Link href="/admin/colaboradores" className="rounded-full border border-[#242017] px-4 py-2 text-sm font-bold">Usuários</Link></div>}>
    <section className="grid gap-6 xl:grid-cols-[1fr_.9fr]">
      <form className="admin-card grid gap-4 p-6" onSubmit={event => { event.preventDefault(); const normalizedSlug = slug || slugify(name); if (!normalizedSlug) return toast.error("Informe um nome válido para gerar o identificador do parceiro."); create.mutate({ displayName: name, slug: normalizedSlug, description: description || null, publicVisibility: false, status: "Rascunho" }); }}>
        <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Novo parceiro</p><h2 className="mt-2 font-serif text-3xl">Começar em rascunho.</h2><p className="mt-2 text-sm leading-6 text-[#655e52]">Um parceiro não recebe alcance público nem acesso territorial até que você associe territórios, membros autorizados e ative o registro.</p></div>
        ...
        <Button className="bg-[#242017] text-white" disabled={create.isPending}>{create.isPending ? "Criando..." : "Criar parceiro em rascunho"}</Button>
      </form>
      <aside className="admin-card p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Regra de ouro</p><h2 className="mt-2 font-serif text-3xl">Território não é um rótulo decorativo.</h2><div className="mt-5 grid gap-4 text-sm leading-6 text-[#655e52]"><p>O parceiro só poderá operar registros vinculados ao seu território autorizado. A plataforma mantém contas, infraestrutura, Home nacional, políticas e consolidados sob governança do Super Admin.</p><p>Identidade territorial pode ser ativada no perfil do parceiro, mas a Home permanece curada pela Ojú — não é um mural de marcas.</p><p>Dados históricos continuam como operação central até você classificá-los explicitamente; nenhuma associação retroativa é feita automaticamente.</p></div></aside>
    </section>
    <section className="mt-8"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Rede cadastrada</p><h2 className="mt-2 font-serif text-3xl">Escopo, pessoas e ativação.</h2></div>
      {partners.isLoading ? <p className="mt-6 text-sm text-[#655e52]">Carregando parceiros...</p> : partners.data?.length ? <div className="mt-5 grid gap-5">{partners.data.map(partner => {
        const selectedTerritory = territoryByPartner[partner.id];
        const selectedMember = memberByPartner[partner.id];
        return <article key={partner.id} className="admin-card p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">{partner.status}</p><h3 className="mt-2 font-serif text-3xl">{partner.displayName}</h3><p className="mt-1 text-sm text-[#655e52]">/{partner.slug} · versão {partner.version}</p></div><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={partner.publicVisibility} onChange={event => update.mutate({ id: partner.id, expectedVersion: partner.version, publicVisibility: event.target.checked })} disabled={update.isPending} />Identidade pública contextual</label></div>
          <p className="mt-3 max-w-2xl text-xs leading-5 text-[#655e52]">O @ só aparece no diretório e no crédito com identidade pública ligada. Não entra na Home nacional.</p>
