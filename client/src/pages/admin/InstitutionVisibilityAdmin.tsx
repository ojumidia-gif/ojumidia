...
export default function InstitutionVisibilityAdmin() {
  ...
  return <AdminPage eyebrow="Sustentação transparente" title="Visibilidade institucional renovável.">
    ...
    <section className="admin-card mt-7 p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Distribuição das receitas ativas</p><h2 className="mt-2 font-serif text-3xl">Para onde vai cada valor líquido.</h2></div><p className="text-sm text-oju-terra-suave">Base atual: <b>{formatMoney(distributionTotal)}</b></p></div>{distributionTotal > 0 ? <><div className="mt-6 flex h-7 overflow-hidden rounded-full bg-oju-papel" aria-label="Distribuição proporcional de receitas">{distribution.filter(item => item.value > 0).map(item => <div key={item.label} title={`${item.label}: ${formatMoney(item.value)}`} style={{ width: `${(item.value / distributionTotal) * 100}%`, background: item.color }} />)}</div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{distribution.map(item => <div key={item.label} className="flex items-center justify-between gap-3 text-sm"><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full" style={{ background: item.color }} />{item.label}</span><b>{formatMoney(item.value)}</b></div>)}</div></> : <p className="mt-5 text-sm leading-6 text-oju-terra-suave">A barra aparecerá quando houver uma vigência ativa com receita líquida confirmada. Nenhum valor é simulado.</p>}</section>
    ...
    <section className="mt-7 grid gap-7 xl:grid-cols-[420px_1fr]">
      <form className="admin-card grid gap-4 p-6" onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); if (hasCaptor && principal && !capturedByUserId) { toast.error("Selecione o administrador responsável pela captação ou desmarque a opção."); return; } create.mutate({ institutionId: Number(data.get("institutionId")), plan, startsAt: new Date(`${startsAt}T12:00:00`), expiresAt: new Date(`${expiresAt}T12:00:00`), grossAmount: Number(grossAmount), netAmount: Number(netAmount), paymentReference: String(data.get("paymentReference")) || null, capturedByUserId: hasCaptor ? (principal ? Number(capturedByUserId) : undefined) : null }); }}>
        <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Nova vigência</p><h2 className="mt-2 font-serif text-3xl">Registrar contribuição</h2><p className="mt-2 text-sm leading-6 text-oju-terra-suave">O cadastro fica aguardando confirmação. Só então passa a ter visibilidade ativa.</p></div>
        ...
        <p className="-mt-2 text-xs leading-5 text-oju-terra-suave">{plans[plan].description}</p>
        ...
        {principal && hasCaptor ? <label className="grid gap-2 text-sm font-medium">Administrador responsável pela captação<select required value={capturedByUserId} onChange={event => setCapturedByUserId(event.target.value)} className="h-10 rounded border bg-white px-3"><option value="">Selecione o administrador</option>{captors.data?.map(person => <option key={person.id} value={person.id}>{person.name || person.email}</option>)}</select></label> : hasCaptor ? <p className="-mt-1 text-xs leading-5 text-oju-terra-suave">A participação de captação será atribuída automaticamente à sua própria carteira.</p> : null}
        ...
        <Button disabled={create.isPending || !institutions.data?.length} className="bg-oju-verde text-oju-branco">{create.isPending ? "Registrando..." : "Registrar vigência"}</Button>
      </form>
      <div><div className="mb-4 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#806817]">Carteira de vigências</p><h2 className="mt-2 font-serif text-3xl">Ativas, vencidas e pendentes</h2></div><label className="flex items-center gap-2 text-sm text-oju-terra-suave"><Filter className="h-4 w-4" /><span className="sr-only">Filtrar vigências</span><select value={expiryFilter} onChange={event => setExpiryFilter(event.target.value as ExpiryFilter)} className="h-10 rounded border bg-white px-3 text-oju-terra"><option value="todos">Todas as vigências</option><option value="ativos">Visibilidade ativa</option><option value="vence7">Vence em até 7 dias</option><option value="vence30">Vence em até 30 dias</option><option value="vencidos">Vencidas</option><option value="pendentes">Aguardando confirmação</option></select></label></div>{filteredEntries.length ? <div className="grid gap-4">{filteredEntries.map(item => { const remaining = daysUntil(item.expiresAt); const attention = item.isCurrentlyVisible && remaining >= 0 && remaining <= 7; return <article key={item.id} className={`admin-card p-5 ${attention ? "border-[#b95032]/45 bg-[#fffaf5]" : ""}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#806817]">{item.plan} · {item.effectiveStatus}</p><h3 className="mt-2 font-serif text-2xl">{item.institutionName}</h3><p className="mt-2 text-sm text-oju-terra-suave">{item.institutionType} · vigência até {new Date(item.expiresAt).toLocaleDateString("pt-BR")}{item.isCurrentlyVisible ? ` · ${remaining === 0 ? "vence hoje" : `${remaining} dias restantes`}` : ""}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${attention ? "bg-[#f8d7ca] text-[#933e26]" : item.isCurrentlyVisible ? "bg-[#d6ead4] text-[#2f7040]" : "bg-oju-papel text-oju-terra-suave"}`}>{attention ? "Renovação próxima" : item.isCurrentlyVisible ? "Visibilidade ativa" : item.effectiveStatus}</span></div><div className="mt-4 grid gap-2 text-xs text-oju-terra-suave sm:grid-cols-4"><span>Líquido: <b>{formatMoney(item.netAmount)}</b></span><span>Ojú: <b>{formatMoney(item.ojuAmount)}</b></span><span>Desenv.: <b>{formatMoney(item.developmentAmount)}</b></span><span>Captação: <b>{formatMoney(item.captorAmount)}</b>{item.capturedByName ? ` · ${item.capturedByName}` : " · reserva operacional"}</span></div><div className="mt-5 flex flex-wrap gap-2">{item.effectiveStatus === "Aguardando confirmação" ? <Button size="sm" onClick={() => confirm.mutate({ id: item.id, paymentReference: item.paymentReference })} disabled={confirm.isPending}>Confirmar recebimento</Button> : null}{principal && item.capturedByName ? <label className="inline-flex items-center gap-2 rounded-md border border-oju-terra/15 px-3 py-1.5 text-xs font-medium text-[#393328]">Repasse<select value={item.captorPayoutStatus} onChange={event => updatePayout.mutate({ id: item.id, captorPayoutStatus: event.target.value as "Pendente" | "Parcial" | "Pago" })} disabled={updatePayout.isPending} className="bg-transparent text-xs"><option>Pendente</option><option>Parcial</option><option>Pago</option></select></label> : null}{item.effectiveStatus === "Ativa" ? <Button size="sm" variant="outline" onClick={() => cancel.mutate({ id: item.id })} disabled={cancel.isPending}>Cancelar visibilidade</Button> : null}</div></article>; })}</div> : <EmptyAdmin text="Nenhuma vigência corresponde ao filtro. Crie uma somente após o perfil comunitário estar autorizado." />}</div>
    </section>
  </AdminPage>;
}
