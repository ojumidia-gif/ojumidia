...
function HomeMiniclipsDesk() {
  ...
  return (
    <AdminPage eyebrow="Super Admin · Home nacional" title="Miniclipes do fundo vivo.">
      ...
      <section className="admin-card mt-7 p-6"><p className="editorial-kicker">Super Admin</p><h2 className="mt-2 font-serif text-3xl">Tempo entre miniclipes</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#655e52]">A Home mostra no máximo {HOME_MINICLIP_DISPLAY_SECONDS} segundos de cada miniclipe, com transição de {HOME_MINICLIP_TRANSITION_MS} milissegundos. O arquivo pode ter até {HOME_MINICLIP_MAX_DURATION_SECONDS} segundos; o restante só aparece em Assistir miniclipe, em outra aba. Esta regra não é editável.</p>{curation ? <p className="mt-4 text-sm text-[#655e52]">Curadoria anônima desta sessão pública: {curation.watch} assistir · {curation.mute} mudo · {curation.unmute} som. Sem cookie e sem identidade.</p> : null}</section>
