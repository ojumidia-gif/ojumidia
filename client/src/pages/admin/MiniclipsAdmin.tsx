...
export default function MiniclipsAdmin() {
  ...
  return <AdminPage eyebrow="Super Admin · Home nacional" title="Miniclipes do fundo vivo.">
    <p className="-mt-4 mb-6 max-w-3xl text-sm leading-6 text-[#655e52]">Esta sequência alimenta só a Home nacional. Parceiros não curam este fundo: o território publica conteúdo, não a vitrine da entrada do portal. Publicações editoriais continuam com até 5 fotos e 2 miniclipes; o fundo vivo é outra fila, com até {HOME_MINICLIP_SEQUENCE_LIMIT} clipes.</p>
    ...
    {user?.role === "administrador principal" && <section className="admin-card mt-7 p-6"><p className="editorial-kicker">Super Admin</p><h2 className="mt-2 font-serif text-3xl">Tempo entre miniclipes</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#655e52]">A Home mostra no máximo {HOME_MINICLIP_DISPLAY_SECONDS} segundos de cada miniclipe, com transição de {HOME_MINICLIP_TRANSITION_MS} milissegundos. O arquivo pode ter até {HOME_MINICLIP_MAX_DURATION_SECONDS} segundos; o restante só aparece em Assistir miniclipe, em outra aba. Esta regra não é editável.</p>{curation ? <p className="mt-4 text-sm text-[#655e52]">Curadoria anônima desta sessão pública: {curation.watch} assistir · {curation.mute} mudo · {curation.unmute} som. Sem cookie e sem identidade.</p> : null}</section>}
