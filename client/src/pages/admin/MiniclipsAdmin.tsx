...
function ProductionMiniclipsDesk() {
  ...
  return (
    <AdminPage eyebrow="Produção e monetização" title="Miniclipes da contratação.">
      <p className="-mt-4 mb-6 max-w-3xl text-sm leading-6 text-[#655e52]">Você publica o vídeo curto (até 60s) e liga à cobertura paga. Isso não muda a abertura da Home. A Equipe Ojú é quem escolhe o fundo vivo nacional.</p>
      ...
      <section className="admin-card p-6">
        ...
        <form onSubmit={upload} className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="flex flex-wrap justify-end gap-3 md:col-span-2">
            ...
            <Button disabled={uploading || create.isPending} className="bg-[#242017] text-white"><UploadCloud className="mr-2 h-4 w-4" />{uploading ? "Enviando..." : "Guardar miniclipe"}</Button>
          </div>
        </form>
      </section>
          ...
          <div className="mt-5 grid gap-5 md:grid-cols-2">
            {clips.map(clip => (
              <article key={clip.id} className="admin-card overflow-hidden">
                <video muted loop playsInline controls className="aspect-video w-full bg-[#242017]" src={clip.assetUrl} />
                <div className="p-5">
                  <p className="font-medium">{clip.filename || "Miniclipe sem nome"}</p>
                  <p className="mt-1 text-xs text-[#655e52]">{clip.credit} · {clip.durationSeconds}s</p>
                </div>
              </article>
            ))}
          </div>
...
function HomeMiniclipsDesk() {
  ...
  return (
    <AdminPage eyebrow="Equipe Ojú · Home nacional" title="Miniclipes do fundo vivo.">
      <p className="-mt-4 mb-6 max-w-3xl text-sm leading-6 text-[#655e52]">Esta sequência alimenta só a Home nacional. Admin comum publica miniclipe de contratação; não cura este fundo. Publicações editoriais continuam com até 5 fotos e 2 miniclipes; o fundo vivo é outra fila, com até {HOME_MINICLIP_SEQUENCE_LIMIT} clipes.</p>
      <section className="admin-card p-6"><div className="flex gap-4"><div className="rounded-xl bg-[#f6d978] p-3"><Film className="h-5 w-5" /></div><div><p className="font-serif text-2xl">Adicionar miniclipe</p><p className="mt-1 max-w-2xl text-sm leading-6 text-[#655e52]">Esta área é só para miniclipe do fundo vivo — não misture com foto ou vídeo de cobertura. A sequência comporta até quatro vídeos; a Home mostra um por vez.</p></div></div><form onSubmit={upload} className="mt-6 grid gap-4 md:grid-cols-2"><div className="grid gap-3 md:col-span-2"><MediaAddButton accept="video/*" multiple={false} label="+ Adicionar miniclipe" counts={{ photos: `Miniclipe ${stage.items.length} / 1`, miniclips: `Na sequência: ${clips?.filter(clip => clip.backgroundEligible).length || 0} / 4` }} onFiles={files => stage.addFiles(files.slice(0, 1))} /><MediaStage items={stage.items} onRemove={stage.remove} onRetry={itemId => { const item = stage.items.find(entry => entry.localId === itemId); if (item) void stage.uploadOne(item).catch(error => toast.error(error instanceof Error ? error.message : "Falha no reenvio.")); }} /></div><label className="grid gap-2 text-sm font-medium">Origem<Input required value={origin} onChange={event => setOrigin(event.target.value)} placeholder="Ex.: Produção original Ojú Mídia" /></label><label className="grid gap-2 text-sm font-medium">Crédito / autor<Input required value={credit} onChange={event => setCredit(event.target.value)} placeholder="Nome para crédito" /></label><label className="grid gap-2 text-sm font-medium">Ordem na sequência<Input required min="0" max="99" type="number" value={priority} onChange={event => setPriority(event.target.value)} /></label><label className="grid gap-2 text-sm font-medium md:col-span-2">Faixa de legenda opcional (.vtt)<Input value={captionTrackUrl} onChange={event => setCaptionTrackUrl(event.target.value)} placeholder="/media-storage/.../miniclipe.vtt" /><span className="text-xs font-normal leading-5 text-[#655e52]">Somente texto autorizado pela casa. Não use ponto, canto ou nomes sagrados como decoração.</span></label><p className="self-end text-xs leading-5 text-[#655e52]">A sequência comporta até quatro miniclipes; o maior número aparece primeiro. Remover da sequência não apaga o vídeo do Acervo.</p><div className="flex justify-end md:col-span-2"><Button disabled={uploading || create.isPending} className="bg-[#242017] text-white"><UploadCloud className="mr-2 h-4 w-4" />{uploading ? "Enviando..." : "Adicionar à sequência"}</Button></div></form></section>
      <section className="admin-card mt-7 p-6"><p className="editorial-kicker">Equipe Ojú</p><h2 className="mt-2 font-serif text-3xl">Tempo entre miniclipes</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#655e52]">A Home mostra no máximo {HOME_MINICLIP_DISPLAY_SECONDS} segundos de cada miniclipe, com transição de {HOME_MINICLIP_TRANSITION_MS} milissegundos. O arquivo pode ter até {HOME_MINICLIP_MAX_DURATION_SECONDS} segundos; o restante só aparece em Assistir miniclipe, em outra aba. Esta regra não é editável.</p>{curation ? <p className="mt-4 text-sm text-[#655e52]">Curadoria anônima desta sessão pública: {curation.watch} assistir · {curation.mute} mudo · {curation.unmute} som. Sem cookie e sem identidade.</p> : null}</section>
      <section className="mt-7"><div className="mb-4"><p className="editorial-kicker">Fundo vivo atual</p><h2 className="mt-2 font-serif text-3xl">Sequência de miniclipes</h2></div>{isLoading ? <p>Carregando miniclipes...</p> : clips?.length ? <div className="grid gap-5 md:grid-cols-2">{clips.map(clip => <article key={clip.id} className="admin-card overflow-hidden"><video muted loop playsInline controls className="aspect-video w-full bg-[#242017]" src={clip.assetUrl} /><div className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">{clip.filename || "Miniclipe sem nome"}</p><p className="mt-1 text-xs text-[#655e52]">{clip.credit} · ordem {clip.backgroundPriority}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${clip.backgroundEligible ? "bg-[#d8eadc] text-[#2c683b]" : "bg-[#eee9dc] text-[#655e52]"}`}>{clip.backgroundEligible ? "Na sequência" : "Guardado no Acervo"}</span></div><p className="mt-4 text-sm text-[#655e52]">Origem: {clip.origin}</p><div className="mt-4 grid gap-2"><label className="text-xs font-medium text-[#655e52]">Legenda .vtt autorizada<Input className="mt-1" value={clipCaptions[clip.id] ?? ""} onChange={event => setClipCaptions(current => ({ ...current, [clip.id]: event.target.value }))} placeholder="/media-storage/.../miniclipe.vtt" /></label><Button size="sm" variant="outline" disabled={setCaption.isPending} onClick={() => setCaption.mutate({ id: clip.id, captionTrackUrl: clipCaptions[clip.id]?.trim() || undefined })}>Salvar legenda</Button></div><div className="mt-5 flex gap-2">{clip.backgroundEligible ? <Button size="sm" variant="outline" disabled={setBackground.isPending} onClick={() => setBackground.mutate({ id: clip.id, active: false })}>Remover da sequência</Button> : <Button size="sm" disabled={setBackground.isPending} onClick={() => setBackground.mutate({ id: clip.id, active: true, priority: clip.backgroundPriority || 99 })}>Adicionar à sequência</Button>}</div></div></article>)}</div> : <EmptyAdmin text="Ainda não há miniclipes. Envie o primeiro vídeo para estabelecer o fundo vivo documental da Home." />}</section>
    </AdminPage>
  );
}
