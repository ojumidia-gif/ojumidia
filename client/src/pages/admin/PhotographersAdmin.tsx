    ...
    <AdminPage eyebrow="Portal → Fotógrafos" title="Cadastrar e publicar fichas.">
      ...
      <section className="grid gap-7 xl:grid-cols-[380px_1fr]">
        <form className="admin-card grid gap-3 p-5" onSubmit={async event => {
          ...
          <details className="rounded-xl bg-oju-papel px-3 py-2 text-sm">
          ...
          <Button disabled={createExecutor.isPending || updateExecutor.isPending} className="bg-oju-verde text-oju-branco">{canPublish ? "Cadastrar e publicar" : "Cadastrar"}</Button>
        </form>
        <div className="grid gap-3">
          {isLoading ? <p className="text-sm text-oju-terra-suave">Carregando fichas...</p> : executors?.length ? executors.map(item => (
