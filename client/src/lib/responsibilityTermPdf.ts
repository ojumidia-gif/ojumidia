...
export function downloadResponsibilityTermPdf(input: { termId: number; administratorName?: string | null; administratorEmail: string; note?: string | null }) {
  ...
  write("OJÚ MÍDIA", 12, true, 2); write("TERMO DE RESPONSABILIDADE ADMINISTRATIVA", 18, true, 8);
  write(`Referência do termo: OJU-AR-1.0-${input.termId}`, 9, false, 8);
  write("1. Identificação", 12, true, 3);
  write(`Administrador(a): ${clean(input.administratorName)}. Conta Google autorizada: ${input.administratorEmail}. Observação de escopo: ${clean(input.note)}.`, 10, false, 6);
  write("2. Natureza da função", 12, true, 3);
  write("A função administrativa é uma confiança individual e revogável. Ela não transfere propriedade da marca Ojú Mídia, do acervo, das relações institucionais, das credenciais, dos dados pessoais ou das decisões de governança do administrador principal.", 10, false, 6);
  write("3. Responsabilidade editorial, cultural e de privacidade", 12, true, 3);
  write("O(a) administrador(a) compromete-se a respeitar o ciclo editorial, os consentimentos registrados, as restrições culturais, os direitos de imagem e os limites de acesso da própria carteira. Nenhuma contratação torna uma publicação automaticamente pública; materiais privados só podem seguir para curadoria quando houver autorização editorial válida e específica.", 10, false, 6);
  write("4. Dados, documentos e armazenamento", 12, true, 3);
  write("Documentos contratuais, termos assinados e mídias restritas são confidenciais. Devem permanecer nos fluxos protegidos da Ojú, sem cópias, compartilhamentos, downloads ou uso externo não autorizado. Qualquer incidente, acesso indevido ou pedido de revogação deve ser comunicado imediatamente ao administrador principal.", 10, false, 6);
  write("5. Carteira e divisão de receita", 12, true, 3);
  write("Cada fechamento deve ser registrado e vinculado à carteira responsável. Salvo porcentagem diferente formalizada em contrato, a distribuição padrão é 70% para o(a) administrador(a) responsável pela captação ou gestão e 30% para a Ojú Mídia como taxa de operação, infraestrutura, curadoria e manutenção. Valores, repasses e exceções ficam sujeitos à conferência e registro contratual.", 10, false, 6);
  ...
  write("Este PDF deve ser assinado exclusivamente pelo serviço oficial de assinatura eletrônica do gov.br. Após a assinatura, o PDF assinado deve ser entregue ao administrador principal da Ojú para anexo no Centro Administrativo. A Ojú não coleta assinatura eletrônica dentro do portal.", 10, false, 14);
  write("Assinatura do(a) administrador(a) via gov.br: __________________________________________", 10, false, 8);
